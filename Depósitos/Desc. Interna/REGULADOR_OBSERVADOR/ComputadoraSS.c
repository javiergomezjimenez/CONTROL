...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetNumDiscStates(    S, 3);      /****** MODIFICADO *******/ /* Número de estados discretos: 3 */

    if (!ssSetNumInputPorts(S, 2)) return;      /**** MODIFICADO*****/  /* 1 puerto de entrada (Puertos: 0) */
    ssSetInputPortWidth(S, 0, 1);               /**** MODIFICADO*****/  /* Dimensión del puerto de entrada 0: 3 entradas */
    ssSetInputPortDirectFeedThrough(S, 0, 1);   /* y 1 salida directa desde el puerto 0 */
    ssSetInputPortWidth(S, 1, 1);               /****** AÑADIDO  ********/
    ssSetInputPortDirectFeedThrough(S, 1, 1);   /****** AÑADIDO  ********/
...
static void mdlInitializeConditions(SimStruct *S)
{
 ...
 FLAG_INIT = 1;   
      ...
      real_T *Xk_mu0 = ssGetRealDiscStates(S);   /******* AÑADIDO *******/
    int_T  lp;                              /******* AÑADIDO *******/ 

    for (lp=0;lp<2;lp++) {                  /******* AÑADIDO *******/ 
        *Xk_mu0++=0.0;                          /******* AÑADIDO *******/ 
    }
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
 /***** NO MODIFICAR  ****/
  ...
  real_T *Xk_mu  = ssGetRealDiscStates(S);  /******* AÑADIDO *******/
  
  InputRealPtrsType uPtrs0 = ssGetInputPortRealSignalPtrs(S,0);      /* Accede a la entrada 0 vía un puntero */
   InputRealPtrsType uPtrs1 = ssGetInputPortRealSignalPtrs(S,1);  /******* AÑADIDO *******/
  int_T  *iwork = ssGetIWork(S); /* Puntero al vector de n. enteros */
